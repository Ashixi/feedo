...
enum SwarmCommand {
    ...
    InitiateSemanticSearch(String, u32, Option<Vec<String>>, Option<String>, Option<String>, oneshot::Sender<String>),
    ...
    DhtDelete(String, oneshot::Sender<bool>),
...
async fn handle_publish(
    ...
    if payload.source_type.as_deref() != Some("nostr") {
...
async fn handle_dht_delete(
    State(tx): State<mpsc::UnboundedSender<SwarmCommand>>,
    Path(hash): Path<String>,
) -> Result<Json<bool>, (axum::http::StatusCode, String)> {
    let (resp_tx, resp_rx) = oneshot::channel();
    let _ = tx.send(SwarmCommand::DhtDelete(hash, resp_tx));
    match resp_rx.await {
        Ok(data) => Ok(Json(data)),
        _ => Err((axum::http::StatusCode::INTERNAL_SERVER_ERROR, "Failed to delete".to_string())),
    }
}
...
async fn handle_semantic_search(
    ...
    let (resp_tx, resp_rx) = oneshot::channel();
    let _ = tx.send(SwarmCommand::InitiateSemanticSearch(req.text_query, req.limit.unwrap_or(10), req.source_types, req.item_type, req.website, resp_tx));
    if let Ok(res_str) = resp_rx.await {
        if let Ok(json) = serde_json::from_str(&res_str) {
            return Json(json);
        }
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let app = Router::new()
        ...
        .route("/local/delete_hash/:hash", post(handle_dht_delete))
    ...
    loop {
        tokio::select! {
            Some(cmd) = api_rx.recv() => match cmd {
                ...
                SwarmCommand::DhtDelete(hash_str, sender) => {
                    swarm.behaviour_mut().kademlia.store_mut().remove(&kad::RecordKey::new(&hash_str));
                    swarm.behaviour_mut().kademlia.stop_providing(&kad::RecordKey::new(&hash_str));
                    println!("DHT record deleted: {}", hash_str);
                ...
                SwarmCommand::InitiateSemanticSearch(query_text, limit, source_types, item_type, website, sender) => {
                    let mut hasher = Sha256::new();
