...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  Future<void> _fetchProfileAndPosts() async {
    ...
    if (widget.pubkey.startsWith('nprofile')) {
      final decoded = Bech32.decodeProfile(widget.pubkey);
      relaySet.addAll(decoded.relays);
    }
            ...
            if (data[0] == 'EVENT') {
              ...
              if (data[1] == profileReqId) {
                try {
                  ...
                  // Backfill backend immediately
                  try {
                    final String apiUrl = Constants.apiUrl;
                    http.put(
                      Uri.parse('$apiUrl/v1/identity/update/$pk'),
                      headers: {'Content-Type': 'application/json'},
                      body: jsonEncode({
                        'metadata': content,
                        'signature': 'from_ui_auto_backfill'
                      }),
