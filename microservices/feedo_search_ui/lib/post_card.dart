...
class _PostCardState extends State<PostCard> {
  ...
  Future<void> _checkAndLoadMissingContent() async {
    ...
    final authorAddress = widget.post['author_address'] ?? widget.post['pubkey'] ?? widget.post['author'];
  ...
  void _openUserProfile() {
    final authorAddress = widget.post['author_address'] ?? widget.post['pubkey'] ?? widget.post['author'] ?? 'Unknown';
  ...
  Future<void> _handleLike() async {
    ...
    final authorPubkey = widget.post['author_address'] ?? widget.post['pubkey'] ?? widget.post['author'];
  ...
  Widget build(BuildContext context) {
    ...
    final String authorAddress = widget.post['author_address'] ?? widget.post['pubkey'] ?? widget.post['author'] ?? 'Unknown';
    String authorName = widget.post['author_name'] ?? authorAddress;
    if (authorName == authorAddress && authorAddress.length >= 64) {
      authorName = '${authorAddress.substring(0, 8)}...${authorAddress.substring(authorAddress.length - 8)}';
    }
    ...
    if (itemType == 'profile') {
      String profileName = widget.post['display_author'] ?? widget.post['author_name'] ?? authorName;
