          ...
          if (msg[0] == 'EVENT' && msg[1] == subId) {
            ...
            if (kind == 0) {
                try {
                  ...
                  // Automatically push to backend so it caches the profile for the future
                  try {
                    final String apiUrl = Constants.apiUrl;
                    http.put(
                      Uri.parse('$apiUrl/v1/identity/update/${ev['pubkey']}'),
                      headers: {'Content-Type': 'application/json'},
                      body: jsonEncode({
                        'metadata': content,
                        'signature': 'from_ui_auto_backfill_resolver'
                      }),
