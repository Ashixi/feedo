import { FeedoClient } from './sdk/typescript/src/client';
...
async function runTest() {
    ...
    // 2. Initialize the Feedo SDK Client
    const client = new FeedoClient({
        storageNodeUrl: 'http://127.0.0.1:3011',
        consensusNodeUrl: 'http://127.0.0.1:3012',
        searchNodeUrl: 'http://127.0.0.1:8013',
        privateKey: wallet.privateKey
    });
    try {
        console.log("\n[1] Registering DID on Consensus Node...");
        const signature = await wallet.signMessage("register");
        await client.consensus.registerDid(wallet.publicKey, signature);
        ...
        console.log(`✅ Uploaded successfully! HashID: ${hashId}`);
        console.log("\n[3] Waiting 5 seconds for Search Node indexing...");
        await new Promise(r => setTimeout(r, 5000));
        console.log("\n[4] Searching for the post by app_id...");
        const searchResults = await client.search("Hello", 10, true, "all", 0, "com.myawesomeapp");
