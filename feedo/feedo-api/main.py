...
def _display_author_name(author: User | None, fallback_wallet: str | None = None) -> str:
    if author is not None:
        for value in (author.username, author.display_name, author.public_id):
...
def _author_avatar_url(author: User | None) -> str | None:
    if author is None or not getattr(author, "avatar_media_hash", None):
        return None
    return f"/p2p-media/{author.avatar_media_hash}"
...
async def _serialize_post_for_client(db: AsyncSession, post: Post) -> dict[str, Any]:
    ...
    display_author = (
        ...
        if post.original_author_name
        else _display_author_name(author, post.author_address)
    )
...
async def process_incoming_post(db: AsyncSession, p_data: dict, check_chain: bool = True):
    ...
    if not parent_id and not exact_match and source_type != "media_blob":
        lang = p_data.get("language") or new_post.language or ""
...
async def proxy_local_publish(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        pb_req = feedo_pb2.PublishRequest(
            ...
            content_blob_hash=payload["content_blob_hash"],
            ...
            metadata=json.dumps(payload.get("metadata_", {})),
        ...
        if response.status_code in (200, 201):
            ...
            if hash_id:
                ...
                for _ in range(25):
                    stmt = (
                        select(Post)
                        .options(selectinload(Post.author), selectinload(Post.duplicates).selectinload(Post.author))
                        .where(Post.hash_id == hash_id)
                    )
                ...
                if post:
                        # If client provided a client_id in payload metadata_, persist it
                        try:
                            client_meta = (payload.get('metadata_', {}) or {})
                        ...
                        return {
                        "status": "published",
                        "post": await _serialize_post_for_client(db, post),
                    }
...
async def get_feed(limit: int = 50, offset: int = 0, source_type: str = "main", wallet_address: str | None = None, request: Request = None, db: AsyncSession = Depends(get_db)):
    ...
    stmt = select(Post).where(Post.id.in_(post_ids_to_fetch)).options(
